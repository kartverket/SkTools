CREATE OR REPLACE FORCE VIEW "@historikk_index1_db_username@"."KOMMUNERFORSTED"
(
  "STEDID",
  "KOMMUNEID",
  "TVERSION",
...
AS
  SELECT
    ...
    h."TBEGIN",
    h."TEND"
  FROM "@historikk_index1_db_schema@"."KOMMUNERFORSTED_H" h
  WHERE
...
CREATE OR REPLACE TRIGGER "@historikk_index1_db_schema@"."T_H_KOMMUNERFORSTED"
  INSTEAD OF INSERT OR UPDATE OR DELETE ON "@historikk_index1_db_username@"."KOMMUNERFORSTED"
  FOR EACH ROW
  DECLARE
...
BEGIN
  ...
  END IF;
  IF INSERTING THEN
    INSERT INTO "@historikk_index1_db_schema@"."KOMMUNERFORSTED_H" VALUES
      (
        1,
        t_Trans,
        t_End,
  ...
  ELSIF UPDATING THEN
    IF :old."TBEGIN" < t_Trans THEN
      ...
      UPDATE "@historikk_index1_db_schema@"."KOMMUNERFORSTED_H" SET
  ...
  ELSIF DELETING THEN
    ...
    DELETE FROM "@historikk_index1_db_schema@"."KOMMUNERFORSTED_H"
    WHERE
        "STEDID"    = :old."STEDID"
    AND "KOMMUNEID" = :old."KOMMUNEID"
    AND "TEND"      = t_End
...
ALTER TRIGGER "@historikk_index1_db_schema@"."T_H_KOMMUNERFORSTED" ENABLE;